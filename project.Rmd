...
### From the data above it is very clear that VIX and S&P are the two most inversly correlated, but there is alot of interesting trends. For example, we see that interest rates were fairly inverse to the S&P until the coronavirus situation with led to both declining. We will discuss this further below, and based on the charts above it is obviously hard to tell price changes based on 4 separate charts.If we simply combined them, it would not work as the S&P is well over $2,000 while overs are below $20. The large axis range would distort the smaller priced indexes, so we much adjust each stock so we can somehow see its changes effectively on a reasonable axis. What we did was index each stock to 0 at is a start date and chart the rest based on the percentage change, and this way we eliminate price difference as a problem completely. The equation to do this is (New Price - Initial Price)/Initial Price. You can see this in the MasterData changes below.
...
### Above is the assets all toghether indexed by 0. Initially we can see many more obvious connections but we can look on a case by case basis.
...
MasterData %>%
  ...
  geom_line()
```
### From above you can tell that generally when the S&P dips, the VIX clearly spikes. They are meant to be fairly inversly correlated. It is much more pronounced in the VIX as it is in the nature of the index. It is very obvious for the recent coronavirus situation as the S&P completely dropped while the VIX surged ~30%
