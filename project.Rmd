...
MasterData <-
  ...
  gather(key=asset, value= price, spClose, flClose, gldClose, usoClose, vixClose)
...
## From the data above it is very clear that VIX and S&P are the two most inversly correlated, but there is alot of interesting trends. For example, we see that interest rates were fairly inverse to the S&P until the coronavirus situation with led to both declining. We will discuss this for the full project. Additionally on the full project, we will utilize MasterData and mutate adjusted returns indexed to 0. This will be able to show all the charts on one or one faceted grid with everything in the same range. Returns will also generate a much better idea of correlations rather than just looking at prices.
